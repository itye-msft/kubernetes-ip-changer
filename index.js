...
const settings = {
    LbRecycleLabel: process.env.LbRecycleLabel || "lbrecycle",
...
app.get('/recycle', (request, response, next) => {
    ...
    client.api.v1.services.get()
        .then(function (services) {
            ...
            response.send('Recycled ' + svcs.length + ' Services.');
        })
        .catch(next);
});
...
function getServicesToRecycle(services) {
    ...
    services.body.items.forEach(service => {
        if (settings.LbRecycleLabel != null && settings.LbRecycleLabel != "") {
            for (var key in service.metadata.labels) {
                if (key == "segasec" && service.metadata.labels[key] == settings.LbRecycleLabel) {
                    svcs.push(service);
                }
            }
        }
